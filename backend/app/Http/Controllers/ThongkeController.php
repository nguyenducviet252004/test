...
class ThongkeController extends Controller
{
        ...
        $topProducts = Order_detail::select(
            ...
            ->map(function ($item) {
                ...
                return [
                    ...
                    'image' => $product ? $product->img_thumb : null,
    ...
    public function tonkho(Request $request)
    {
        ...
        $productsInStock = Product::whereHas('variants', function($q) use ($startDate, $endDate) {
            $q->where('quantity', '>', 0);
            if ($startDate && $endDate) {
                $q->whereBetween('created_at', [$startDate, $endDate]);
            }
        })->with(['variants' => function($q) use ($startDate, $endDate) {
            $q->where('quantity', '>', 0);
            if ($startDate && $endDate) {
                $q->whereBetween('created_at', [$startDate, $endDate]);
            }
        ...
        $nearlySoldOut = Product::whereHas('variants', function($q) use ($startDate, $endDate) {
