...
export interface CartItem {
  ...
  size_id: number;
  color_id: number;
  color?: string;  // Thêm tên màu sắc
  size?: string;   // Thêm tên kích thước
  total: number;
  product: {
    id: number;
    name: string;
...
// Thêm sản phẩm vào giỏ hàng (dùng product_variant_id)
export const addToCart = createAsyncThunk<CartItem, { product_variant_id: number; quantity: number }, { rejectValue: { message: string } }>(
  'cart/addToCart',
...
export const clearCart = createAsyncThunk<void, number, { rejectValue: string }>(
  'cart/clearCart',
  async (userId: number, { rejectWithValue }) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.delete(
        `http://127.0.0.1:8000/api/cart/${userId}/clear`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      return response.data;
    } catch (error: any) {
